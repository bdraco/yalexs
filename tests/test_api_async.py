...
async def test_async_get_doorbell_detail(self, httpx_client, respx_mock):
    ...
    respx_mock.get(expected_doorbell_image_url, body="doorbell_image_mocked")
    ...
    self.assertEqual(
        await doorbell.async_get_doorbell_image(httpx_client),
...
async def test_async_unlock(self, httpx_client, respx_mock):
    lock_id = 1234
    respx_mock.put(
        API_UNLOCK_URL.format(lock_id=lock_id), body='{"status": "unlocked"}'
    )
...
async def test_async_unlock_async_new_bridge(self, httpx_client, respx_mock):
    lock_id = 1234
    respx_mock.put(
        f"{API_UNLOCK_ASYNC_URL}{HYPER_BRIDGE_PARAM}".format(lock_id=lock_id)
    )
...
async def test_async_status_async_new_bridge(self, httpx_client, respx_mock):
    lock_id = 1234
    respx_mock.put(
        f"{API_STATUS_ASYNC_URL}{HYPER_BRIDGE_PARAM}".format(lock_id=lock_id)
    )
...
def test__raise_response_exceptions(self):
    ...
    four_two_eight = Response(status_code=428)
    ...
    for status_code in ERROR_MAP:
        mocked_response = Response(status_code=status_code)
